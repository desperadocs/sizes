to:	Gabrielle Long, GABRIELLE@NBSENH
from:	Pete R. Jemian, JEMIAN@ANLPNS.PNS.ANL.GOV
date:	18 August 1992

It was very good to talk with you last week in
Pittsburgh.  Enclosed is the version of the maximum
entropy program that I am now using.  The wavelength smearing
has been turned OFF for now (by setting the parameter
"dWave" to 1.0E-6 and commenting out any requests for the user
to change dWav).  I think that the program is very similar
to the one which you have been using for a while.
The shape function is the ellipsoid function of Shull and 
Roess that was mentioned in the Jemian&Allen paper that we are
working on.  Wavelength smearing is added by increasing the
parameter dWave from its near delta function value.
There is one additional user parameter that you may not
be familiar with.  You are asked how many iterations should
occur between the plots of the residuals and the current f(D).
By setting this number to 1, you will observe the same operation
that you are used to.  If you increase this number to 10 or 15, you
will find the program executes MUCH faster.  It turns out that
the program was spending most of its time writing information to
the screen.  Reducing the number of interim screen plots speeds
things up quite a bit.  Also, this enables the user to concentrate
on the changing values of ChiSqr and Entropy with each iteration.

I now call the program	ROESS.FOR
	which is a local modification of the program MAXE.FOR.
I have not revised the documentation yet.
Will you want that?  It might take some time. Source code follows:
===================================================================
	PROGRAM Roess
...
C ... Define (initially) the default responses
	...
	DATA dWave	/1.0E-6/	! frac. FWHM of wavelength dispersion
C PRJ, 8/92:	dWave=1.0E-6 is effectively a delta-function distribution
C		Thus, wavelength smearing is turned off in this version
C		User is not prompted to change "dWave"
C		Code left in for future use (a feature, not a bug)
C	For NIST reactor, set dWave=0.25	(25% delta lambda / lambda)
...
C		WRITE (ioUnit,63304) dWave
C		WRITE (*,63304) dWave
C	63304	FORMAT (' Wavelength distribution frac. FWHM = ', 
C	     >		F12.5)
...
C		Suggest = dWave
C	   22	WRITE (*,'(A,F,A,$)') ' frac. wavelength dist. FWHM? [A] <', 
C	     >		suggest, '> '
C		READ (*, '(F10.0)') suggest
C		IF (suggest .LT. 0.0  .OR.  suggest .GE. 1.0) GO TO 22
C		IF (suggest .GT. 0.0) dWave = suggest
